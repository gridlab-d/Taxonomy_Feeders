...
ylim([0 1.5]);
my_legend = {'Base';'R&S'};
set_figure_graphics(data_labels,fname,2,my_legend,.02,'northoutside',1,0,'horizontal');
hold off;
close(fname);
...
ylabel('Minutes');
%ylim([-80 00]);
set_figure_graphics(data_labels,fname,1,'none',0.01,'northoutside',1,0,'horizontal');
hold off;
close(fname);
